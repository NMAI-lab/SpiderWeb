...
public class P2PGraphViewer extends JApplet implements EventPlayerListener, NetworkGraphListener {
	...
	private ArrayList<LogEvent> queryLogList;
	...
	private void startGraph() {
		...
		graphStatisticsPane = new GraphStatisticsPanel(graph.getDynamicGraph());
		graph.getDynamicGraph().addStatListener(graphStatisticsPane);
		
		//Setting the List of queries/queryhits
		graphStatisticsPane.setQueryList(queryLogList);
			...
			Thread loadingThread = new Thread(new Runnable() {
				...
				public void run() {
					...
					if(loader.doLoad()) {
						...
						//initializing the query/queryhit list to be sent to GraphStatisticsPanel
						queryLogList = new ArrayList<LogEvent>(loader.getQueryLogList());
