...
import spiderweb.visualizer.StatChangeListener;
import spiderweb.visualizer.StatisticEvent;
...
public class P2PNetworkGraph extends DirectedSparseGraph<P2PVertex, P2PConnection> implements Graph<P2PVertex, P2PConnection> {
	...
	private ArrayList<StatChangeListener> statChanged;
	/**
	 * 
	 */
	public P2PNetworkGraph() {
		super();
		statChanged = new ArrayList<StatChangeListener>();
	}
	
	public void addStatListener(GraphStatisticsPanel stat)
	{
		statChanged.add(stat);
	}
	...
	public void graphEvent(LogEvent evt, boolean forward, P2PNetworkGraph referenceGraph) {
		if(forward) {
			if (evt.getType().equals("online")){
				...
				//Send event to the Statistics Class
				StatisticEvent statEvent = new StatisticEvent(1, evt.getParam(1));
				for(StatChangeListener L: statChanged)
				{
					L.processChange(statEvent);
				}
			} else if (evt.getType().equals("offline")){
				removePeer(evt.getParam(1));
				
				//Send event to the Statistics Class
				StatisticEvent statEvent = new StatisticEvent(2, evt.getParam(1));
				for(StatChangeListener L: statChanged)
				{
					L.processChange(statEvent);
				}
				...
				//Send message to the graphStat Class to say Re-calculate the Diameter of the graph
				for(StatChangeListener L: statChanged)
				{
					L.checkDiameter(evt.getTime(), true);
				}
			...
			if (evt.getType().equals("online")){
				...
				//Send event to the Statistics Class (offline, we're going backwards here)
				StatisticEvent statEvent = new StatisticEvent(2, evt.getParam(1));
				for(StatChangeListener L: statChanged)
				{
					L.processChange(statEvent);
				...
				//Send event to the Statistics Class (online, we're going backwards here)
				StatisticEvent statEvent = new StatisticEvent(1, evt.getParam(1));
				for(StatChangeListener L: statChanged)
				{
					L.processChange(statEvent);
				...
				for(StatChangeListener L: statChanged)
				{
					L.checkDiameter(evt.getTime(), false);
				}
